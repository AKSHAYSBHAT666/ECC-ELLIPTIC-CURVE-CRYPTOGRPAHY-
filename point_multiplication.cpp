...
pair<ll,ll> point_multiply(ll x1,ll y1,ll k)
{
	...
	for(ll j=0;j<32;j++)
	{
		if(((unsigned)(1<<j)&k)==(unsigned)(1<<j)) msb=j;
	}
...
int main()
{
	...
	pair<ll, ll> Nbg=point_multiply(G.first,G.second,privatekey);
	pair<ll, ll> kPb=point_multiply(Nbg.first,Nbg.second,k);
	...
	//b
	pair<ll,ll>b=point_add(G.first,kPb.first, G.second,kPb.second);
	cout<<"b::"<<b.first<<"::"<<b.second<<endl;


	pair<ll, ll> kGnB=point_multiply(kG.first,kG.second,privatekey);
	cout<<"kGnB::"<<kGnB.first<<"::"<<kGnB.second<<endl;

	pair<ll, ll> pm1=point_add(b.first, kGnB.first, b.second, r-kGnB.second);
	cout<<"Plain:"<<pm1.first<<":"<<pm1.second<<endl;
	...
	cout<<"--------ans:--------"<<endl<<ans.first<<"::"<<ans.second<<endl;
